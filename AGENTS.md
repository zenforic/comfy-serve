...
- `--cleanup-input <DIRECTORY>`: Search and remove temporary files uploaded to ComfyUI's input folder.
...
- `--cors-origin <ORIGIN>`: Allowed origin(s) for CORS (comma-separated, or `CORS_ORIGIN` env var). Applied via a `tower_http::cors::CorsLayer` set as the outermost layer on the router, so it answers `OPTIONS` preflight requests directly (before route method-matching, which would otherwise 405 on `OPTIONS` since routes only register their real methods like `GET`/`POST`) and stamps `Access-Control-Allow-Origin` on the actual responses too. `allow_credentials` is intentionally left disabled since auth here is via `Authorization: Bearer` header, not cookies — this lets `allow_methods`/`allow_headers` safely use `Any` instead of a hand-maintained list. If unset, a startup warning is logged and no CORS headers are sent at all, meaning direct browser calls (not routed through something like Cloudflare Access, which has its own separate CORS layer) will fail with a CORS error even though the request reaches and completes on the server.
